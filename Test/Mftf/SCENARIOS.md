...
| `recommendations` content block type (`Model/ContentBlock/Producer/RecommendationProducer.php`)                        | ✅ `Test/Unit/Model/ContentBlock/Producer/RecommendationProducerTest.php` (the producer itself; see the next row for the on-site rendering path, currently unresolved) |
...
| A content block rendered directly on-site (not via a campaign action) — `Block/Frontend/ContentBlock/Render.php` | 🔴 Real MFTF coverage unwritten (`AdminContentBlockRecommendationsOnSiteTest` exists but is deliberately excluded from every CI group — see ROADMAP.md and that test's own description for the two embedding mechanisms tried and why both broke something). `Test/Unit/Block/Frontend/ContentBlock/RenderTest.php` covers the block's own resolution logic in isolation. |
