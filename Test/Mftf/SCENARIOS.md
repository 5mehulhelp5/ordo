...
| A content block rendered directly on-site (not via a campaign action) — `Block/Frontend/ContentBlock/Render.php`, embedded in a real CMS page via the standard `{{block class=...}}` widget directive, for an anonymous visitor | ✅ `AdminContentBlockRecommendationsOnSiteTest` |
